#include <NewPing.h>
#define COR 9// limite para cor
#define CM 30// limite para distância
...
void setup()
{
  ...
  pinMode(MR1, OUTPUT);
  //Serial.println("dentro do setup, 3 segundos aguardando");
  delay(4800);//aguardando a partida começar
 ...
 // Serial.println("inicalizando busca, entrando no while achou)");delay(3000);
  ...
  {//giro para a esquerda
   ...
   // Serial.println("achou =0, roda direita para por causa do giro à esquerda)");delay(3000);
    analogWrite(MR1,128);//para a roda direita
    analogWrite(ML1,190);//acelera a roda esquerda
...
void sensor_chao_preto()
{
  ...
  {//ok, pode buscar oponente / atacar oponente
   ...
   else if (sBordaL>=COR)//esquerdo=BRANCA, vire à direita
   {
    ...
    analogWrite(ML1,0);// ré à direita
    delay(200);
   ...
   if (sBordaR>=COR) //vire à esquerda
   {
    ...
    analogWrite(MR1,0);//ré à esquerda
    analogWrite(ML1,128);
    delay(200);

    //Serial.println("robo para");delay(3000);
    robotstop();//para o robo
   }
...
void loop() 
{//Serial.println("comecei");delay(3000);
sensor_chao_preto();
}
